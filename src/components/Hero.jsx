...
const DashboardMockup = () => {
  ...
  // Generate chart bars based on selected metric
  const chartValues = useMemo(() => {
    const metric = kpiData[selectedMetric];
    if (!metric) return Array(12).fill(0);
    const baseValue = metric.value;
    const distribution = [0.65, 0.85, 0.55, 0.95, 0.75, 1.15, 1.35, 1.05, 1.45, 1.25, 1.55, 0.95];
    const isPercentage = selectedMetric === 'acos';
    const avg = isPercentage ? baseValue : baseValue / 12;
  ...
  return <div className="relative rounded-xl md:rounded-2xl overflow-hidden bg-[#0B0B0F] border border-[#6A00FF]/20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.7)] ring-1 ring-white/10 flex flex-col h-full font-sans text-base min-h-[600px] md:min-h-[800px]">
      {/* Browser Chrome */}
      ...
      <div className="flex flex-1 overflow-hidden relative bg-[#0B0B0F]">
        {/* Sidebar - Ultra compact on mobile */}
        <div className="w-[52px] md:w-64 bg-[#16161a] border-r border-white/5 flex flex-col py-4 md:py-6 gap-2 md:gap-3 shrink-0 z-10 items-center md:items-stretch">
          {navItems.map(item => <button key={item.id} onClick={() => setActiveTab(item.id)} className={cn("h-10 md:h-14 w-10 md:w-auto mx-auto md:mx-4 rounded-lg md:rounded-xl flex items-center justify-center md:justify-start gap-4 md:px-4 transition-all duration-200 group relative", activeTab === item.id ? "bg-[#6A00FF]/10 text-white" : "text-gray-500 hover:text-white hover:bg-white/5")}>
              {activeTab === item.id && <motion.div layoutId="activeTab" className="absolute -left-1 md:left-0 w-1 h-6 md:h-8 bg-[#6A00FF] rounded-r-full" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} exit={{
            opacity: 0
              ...
              <item.icon size={20} className={cn("shrink-0 md:w-6 md:h-6", activeTab === item.id ? "text-[#6A00FF]" : "group-hover:text-gray-300")} />
              <span className={cn("text-base font-medium hidden md:block", activeTab === item.id ? "font-semibold" : "")}>
                {item.label}
              </span>
            </button>)}
        </div>

        {/* Main Panel - Compact Scrollable */}
        <div className="flex-1 bg-[#0B0B0F] p-3 md:p-8 overflow-y-auto custom-scrollbar relative w-full">
          ...
          {/* KPI Grid - Stacked tight on mobile */}
          ...
          {/* Chart Section - Compact but comfortable on mobile */}
          <div className="bg-[#1F1F25] rounded-xl md:rounded-2xl border border-white/5 p-4 md:p-8 flex flex-col relative group min-h-[auto]">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 md:mb-6 gap-3 md:gap-4 z-10 relative">
               <div className="flex flex-row items-center gap-3 w-full md:w-auto justify-between md:justify-start">
                 ...
                 {/* Custom Metric Selector - Compact */}
                 <div className="relative">
                    <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center justify-between gap-2 text-xs md:text-base font-bold bg-[#0B0B0F] border border-white/10 px-3 py-2 md:px-4 md:py-3 rounded-lg md:rounded-xl text-white hover:border-[#6A00FF]/50 transition-colors min-w-[120px] md:min-w-[180px]">
                    ...
                    duration: 0.2
                          ...
                          {Object.keys(kpiData).map(key => <button key={key} onClick={() => {
                      setSelectedMetric(key);
                      setIsDropdownOpen(false);
                    }} className={cn("w-full text-left px-3 py-2.5 md:px-4 md:py-3.5 text-xs md:text-sm font-medium transition-colors border-b border-white/5 last:border-0 flex items-center justify-between", selectedMetric === key ? "text-white bg-[#6A00FF]/10" : "text-gray-400 hover:text-white hover:bg-[#0B0B0F]")}>
                              {kpiData[key].label}
                              {selectedMetric === key && <div className="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-[#6A00FF]" />}
                            </button>)}
                        </motion.div>}
                    </AnimatePresence>
                 </div>
               ...
               {/* Time Range Selector - Compact with better tap targets */}
               <div className="flex bg-[#0B0B0F] rounded-lg p-1 border border-white/5 w-full md:w-auto justify-center md:justify-start gap-0.5 md:gap-1">
                 {['7D', '30D', '90D'].map((d, i) => <button key={d} className={cn("flex-1 md:flex-none text-[11px] md:text-sm font-bold px-2.5 py-1.5 md:px-6 md:py-2.5 rounded md:rounded-lg transition-all whitespace-nowrap", i === 1 ? "bg-[#6A00FF] text-white shadow-lg" : "text-gray-500 hover:text-white hover:bg-white/5")}>
                     {d}
                   </button>)}
               </div>
            </div>
            
            {/* Chart Render Area - Slightly taller on mobile for better tooltips */}
            <div className="relative z-20 w-full h-[240px] md:h-[350px] flex items-end justify-between gap-1.5 md:gap-4 px-1">
              ...
              {chartValues.map((value, i) => <div key={i} className="relative flex-1 h-full flex items-end group/bar z-10">
                   <motion.div key={selectedMetric} initial={{
                height: "4px"
              }} animate={{
                ...
                delay: i * 0.03,
                ease: "easeOut"
              }} onMouseEnter={() => setHoveredBar(i)} onMouseLeave={() => setHoveredBar(null)} className={cn("w-full rounded-t-sm md:rounded-t-lg transition-all duration-200 relative min-w-[6px] md:min-w-[12px]", hoveredBar === i ? "bg-[#FF7A3D] shadow-[0_0_25px_rgba(255,122,61,0.6)] z-20 scale-x-110" : "bg-gradient-to-t from-[#6A00FF]/40 to-[#6A00FF] hover:from-[#6A00FF]/60 hover:to-[#8B3DFF]")}>
                     {/* Floating Tooltip */}
                     <AnimatePresence>
                       {hoveredBar === i && <motion.div initial={{
                    opacity: 0,
                    y: 10,
                    scale: 0.95
                  }} animate={{
                    ...
                    scale: 0.95
                           ...
                           <span className="text-[9px] md:text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Nov {18 + i}</span>
  ...
  y: -2
    ...
    <div className="flex justify-between items-start mb-2 relative z-10">
      <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-[#2d2d35] flex items-center justify-center group-hover:bg-[#6A00FF]/20 transition-colors duration-300">
         {metricKey === 'spend' && <WalletIcon className="w-4 h-4 md:w-6 md:h-6 text-gray-400 group-hover:text-[#6A00FF] transition-colors" />}
         {metricKey === 'sales' && <DollarSign className="w-4 h-4 md:w-6 md:h-6 text-gray-400 group-hover:text-[#6A00FF] transition-colors" />}
         {metricKey === 'orders' && <ShoppingCart className="w-4 h-4 md:w-6 md:h-6 text-gray-400 group-hover:text-[#6A00FF] transition-colors" />}
         {metricKey === 'acos' && <Percent className="w-4 h-4 md:w-6 md:h-6 text-gray-400 group-hover:text-[#6A00FF] transition-colors" />}
      </div>
      <div className={cn("flex items-center text-[10px] md:text-xs font-bold px-1.5 py-1 md:px-2.5 md:py-1.5 rounded-md md:rounded-lg bg-opacity-10", kpi.good ? "text-[#2ECC71] bg-[#2ECC71]/10" : "text-[#FF4D4D] bg-[#FF4D4D]/10")}>
        {kpi.good ? <TrendingDown size={10} className="mr-1 md:mr-1.5" /> : <TrendingUp size={10} className="mr-1 md:mr-1.5" />}
        {kpi.change}
      </div>
    </div>
    <div>
      <div className="text-[10px] md:text-sm text-gray-500 font-bold uppercase tracking-wider mb-1 md:mb-2">{kpi.label}</div>
      <div className="text-xl md:text-4xl font-bold tracking-tight text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-400 transition-all duration-300">
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        <div className="grid lg:grid-cols-1 gap-8 md:gap-16 items-center text-center">
          ...
          ease: [0.22, 1, 0.36, 1]
            ...
            <p className="text-base md:text-2xl text-[#F3F3F4]/80 mb-8 md:mb-14 max-w-3xl mx-auto font-normal leading-relaxed text-balance antialiased">Stop bleeding money on bad bids. Our AI continuously optimizes your KDP campaigns 24/7 to lower ACOS and scale sales, while you sleep.</p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-6 mb-12 md:mb-24 w-full sm:w-auto">
